...
/** FileSystem caching class. Aims to prevent many of the same FileSystem objects being created between copy and delete
  * actions in the same partition.
  */
class FileSystemObjectCacher(hadoopConfiguration: Configuration) {
  ...
  /** Get a FileSystem object based on the given URI if it already exists. If it doesn't exist, create one and store it.
    */
