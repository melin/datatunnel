...
import com.dataworker.datax.api.{DataXException, DataxReader}
import com.dataworker.datax.kafka.util.HudiUtils
...
class KafkaReader extends DataxReader {
  ...
  override def read(sparkSession: SparkSession, options: util.Map[String, String]): Dataset[Row] = {
    ...
    val querySql = "select if(kafka_key is not null, kafka_key, cast(current_timestamp as string)) as id, " +
      "message, kafka_topic, date_format(current_timestamp, 'yyyyMMddHH') ds from " + tmpTable
