 ...
 */
public class HiveSource implements DataTunnelSource<HiveSourceOption> {
    private static final Logger LOGGER = LoggerFactory.getLogger(HiveSource.class);

    @Override
    public Dataset<Row> read(DataTunnelSourceContext<HiveSourceOption> context) throws IOException {
        HiveSourceOption sourceOption = context.getSourceOption();
