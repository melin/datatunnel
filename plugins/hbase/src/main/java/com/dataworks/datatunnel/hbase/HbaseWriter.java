package com.dataworks.datatunnel.hbase;

import com.dataworks.datatunnel.api.DataXException;
import com.dataworks.datatunnel.api.DataxWriter;
import com.dataworks.datatunnel.hbase.constant.MappingMode;
import com.dataworks.datatunnel.hbase.constant.WriteMode;
import com.dataworks.datatunnel.hbase.util.DistCpUtil;
import com.github.melin.superior.jobserver.api.LogUtils;
import com.dataworks.datatunnel.hbase.constant.HbaseWriterOption;
...
public class HbaseWriter implements DataxWriter {
    ...
    public void validateOptions(Map<String, String> options) {
        ...
        String runningMode = options.get(HbaseWriterOption.RUNNING_MODE);
    ...
    public void validateCreateHfileOptions(Map<String, String> options){
        String tableName = options.get(HbaseWriterOption.TABLE_NAME);
        if (StringUtils.isBlank(tableName)){
            throw new DataXException("缺少tableName参数");
        }
        this.tableName = tableName;

        String hfileDir = options.get(HbaseWriterOption.HFILE_DIR);
        if (StringUtils.isBlank(hfileDir)){
            throw new DataXException("缺少hfileDir参数");
        }
        this.hfileDir = hfileDir;

        WriteMode writeMode = EnumUtils.getEnum(WriteMode.class, options.get(HbaseWriterOption.WRITE_MODE));
        if (!Objects.isNull(writeMode)){
            this.writeMode = writeMode;
        }

        MappingMode mappingMode = EnumUtils.getEnum(MappingMode.class, options.get(HbaseWriterOption.MAPPING_MODE));
        ...
        if (MappingMode.arrayZstd.equals(this.mappingMode) || MappingMode.stringConcat.equals(this.mappingMode)){
            String mergeQualifier = options.get(HbaseWriterOption.MERGE_QUALIFIER);
        ...
        if (MappingMode.stringConcat.equals(this.mappingMode)){
            String separator = options.get(HbaseWriterOption.SEPARATOR);
            if (StringUtils.isNotEmpty(separator)){
                this.separator = separator;
            }
        }

        String hfileMaxSize = options.get(HbaseWriterOption.HFILE_MAX_SIZE);
        ...
        String hfileTime = options.get(HbaseWriterOption.HFILE_TIME);
        ...
        String compactionExclude = options.get(HbaseWriterOption.COMPACTION_EXCLUDE);
    ...
    public void validateDistcpOptions(Map<String, String> options){
        String distHfileDir = options.get(HbaseWriterOption.DISTCP_HFILE_DIR);
        ...
        String distcpMaxMaps = options.get(HbaseWriterOption.DISTCP_MAX_MAPS);
        ...
        String distcpPerMapBandwidth = options.get(HbaseWriterOption.DISTCP_PER_MAP_BANDWIDTH);
    ...
    public void write(SparkSession sparkSession, Dataset<Row> dataset, Map<String, String> options) throws IOException {
        ...
        LogUtils.info("dataset count=" + count);
        if (0 == count){
        ...
        LogUtils.info("profile=" + profile);

        logger.info("开始hbaseWriter");
        LogUtils.info("开始hbaseWriter");
    ...
    public void createHfileStep(SparkSession sparkSession, Dataset<Row> dataset, String jobInstanceCode, String profile) throws IOException{
        ...
        LogUtils.info("开始createHfileStep");
        ...
        try {
            ...
            LogUtils.info("hbaseTableMeta=" + hbaseTableMeta);
        } catch (Exception e) {
            logger.error("创建hbaseTableMeta失败", e);
            LogUtils.error("创建hbaseTableMeta失败" + e.getMessage());
        ...
        LogUtils.info("hfile路径=" + stagingDir);
        // hfile生成成功后的路径
        ...
        LogUtils.info("table=" + tableName + " regionSize=" + regionSize);
        ...
        try {
            ...
            LogUtils.info("writeMode=" + writeMode + " mappingMode=" + mappingMode + " hfile生成成功");
        } catch (Exception e) {
            logger.error("hfile生成失败", e);
            LogUtils.error("hfile生成失败," + e.getMessage());
            // 清理目录
            fileSystem.delete(stagingDirPath, true);
            LogUtils.error("清理目录" + stagingDirPath);
        ...
        if (fileSystem.rename(stagingDirPath, stagingDirSuccPath)){
            ...
            LogUtils.info("修改hfile目录名=" + stagingDirSuccPath + "成功");
        } else {
            logger.error("修改hfile目录名={}失败", stagingDirSuccPath);
            LogUtils.error("修改hfile目录名=" + stagingDirSuccPath + "失败");
    ...
    public void distcpStep(SparkSession sparkSession, String jobInstanceCode, String profile){
        ...
        try {
            ...
            LogUtils.info("distCp目录=" + destStagingDir);
            ...
            LogUtils.info("nameServices=" + nameServices);
            ...
            LogUtils.info("sourcePath=" + sourcePath + ",destPath=" + destPath);
