...
class DefaultSource extends TableProvider {
  ...
  override def inferSchema(
    ...
    val (_, fields, partitions) = odpsTablePrerequisite(
  ...
  private def requiredOptions(
    ...
    val odpsAccessKeySecret =
      options.get(OdpsSourceOptions.ODPS_ACCESS_KEY_SECRET)

    if (
      !odpsProject.isEmpty && !odpsTable.isEmpty && !odpsEndpoint.isEmpty && !odpsAccessKeyId.isEmpty && !odpsAccessKeySecret.isEmpty
    ) {
      (
        odpsProject,
        odpsTable,
  ...
  private def odpsTablePrerequisite(
      options: java.util.Map[String, String]
  ): (com.aliyun.odps.Table, List[StructField], List[StructField]) = {
