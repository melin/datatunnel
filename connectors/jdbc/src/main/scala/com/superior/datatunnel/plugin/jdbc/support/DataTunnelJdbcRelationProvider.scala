    ...
    try {
      if (StringUtils.endsWithIgnoreCase(writeMode, "BULKINSERT")) {
        if (dataSourceType.equalsIgnoreCase("POSTGRESQL") || dataSourceType.equalsIgnoreCase("GAUSSDWS")) {
          databaseDialect.bulkInsertTable(conn, df, options, parameters, upsertKeyColumns)
          ...
          saveMode match {
            case SaveMode.Overwrite =>
              if (options.isTruncate && isCascadingTruncateTable(options.url) == Some(false)) {
                ...
                databaseDialect.saveTable(df, tableSchema, isCaseSensitive, options, writeMode, upsertKeyColumns)
              } else {
                // Otherwise, do not truncate the table, instead drop and recreate it
                dropTable(conn, options.table, options)
                createTable(conn, options.table, df.schema, isCaseSensitive, options)
                databaseDialect.saveTable(df, Some(df.schema), isCaseSensitive, options, writeMode, upsertKeyColumns)
              }
