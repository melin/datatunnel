...
object KingbaseDialect extends JdbcDialect with SQLConfHelper {
  ...
  /** The SQL query used to truncate a table. For Postgres, the default behaviour is to also truncate any descendant
    * tables. As this is a (possibly unwanted) side-effect, the Postgres dialect adds 'ONLY' to truncate only the table
    * in question
    * @param table
    *   The table to truncate
    * @param cascade
    *   Whether or not to cascade the truncation. Default value is the value of isCascadingTruncateTable(). Cascading a
    *   truncation will truncate tables with a foreign key relationship to the target table. However, it will not
    *   truncate tables with an inheritance relationship to the target table, as the truncate query always includes
    *   "ONLY" to prevent this behaviour.
    * @return
  ...
  /** java.sql timestamps are measured with millisecond accuracy (from Long.MinValue milliseconds to Long.MaxValue
    * milliseconds), while Spark timestamps are measured at microseconds accuracy. For the "infinity values" in
    * PostgreSQL (represented by big constants), we need clamp them to avoid overflow. If it is not one of the infinity
