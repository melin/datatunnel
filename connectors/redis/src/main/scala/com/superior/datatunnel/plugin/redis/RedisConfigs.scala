...
import org.apache.spark.sql.redis.RedisOptions.{REDIS_DATABASE, REDIS_HOST, REDIS_PASSWORD, REDIS_PORT, REDIS_SSL_ENABLED, REDIS_TIMEOUT, REDIS_USER}
...
case class RedisEndpoint(host: String = Protocol.DEFAULT_HOST,
  ...
  def this(uri: URI) {
    this(uri.getHost, uri.getPort, JedisURIHelper.getUser(uri), JedisURIHelper.getPassword(uri), JedisURIHelper.getDBIndex(uri),
      Protocol.DEFAULT_TIMEOUT, uri.getScheme == "rediss")
  }
